...
class BaseCharacter:
    ...
    def _call_llm(self, filled_prompt: str) -> str:
        ...
        # 以 system + user 的方式组织消息，确保人格与场景牢固注入
        messages: List[Dict[str, str]] = []
        for sys_msg in self.system_prompts:
            if sys_msg:
    ...
    def _sanitize_delta(self, value, *, field_name: str, min_value: int, max_value: int) -> Tuple[int, int]:
        coerced = self._coerce_int(value, default=0, field_name=field_name)
        clamped = max(min(coerced, max_value), min_value)
        if coerced != clamped:
            logger.warning("%s out of bounds (raw=%s, clamped=%s)", field_name, coerced, clamped)
            print(f"[WARN] {field_name} raw={coerced} 超出范围，已限制在 {min_value}~{max_value} 之间 (applied={clamped})")
        return clamped, coerced

    def apply_analysis_to_state(self, analysis: Dict, user_input: str, affection_delta: Optional[int] = None, boredom_delta: Optional[int] = None) -> None:
        if affection_delta is None or boredom_delta is None:
            affection_delta, _ = self._sanitize_delta(
                analysis.get("affection_delta", 0), field_name="affection_delta", min_value=-10, max_value=10
            )
            boredom_delta, _ = self._sanitize_delta(
                analysis.get("boredom_delta", 0), field_name="boredom_delta", min_value=-5, max_value=5
            )
